...
m_srting &m_srting::operator=(const m_srting &_str_first)
{
	...
	/*if (array_chars != nullptr)
		delete[] array_chars;*/
	array_chars = new char[_str_first.get_size_str() + 1];
	size_str = _str_first.get_size_str() + 1;
	for (size_t i = 0; i < size_str; i++)
	{
		get(i) = _str_first.get(i);
	}
	//char *tmp = new char[_str_first.get_size_str() + 1];
...
m_srting operator+(const m_srting &_str_first, const m_srting &_str_second)
{
	m_srting temp;
	size_t count = 0;
	temp.set(_str_first.get_size_str() + _str_second.get_size_str() - 1);
	for (size_t i = 0; i < _str_first.get_size_str() - 1; i++)
	{
		temp.get(i) = _str_first.get(i);
		count++;
	}
	for (size_t i = 0; i < _str_second.get_size_str(); i++)
	{
		temp.get(count) = _str_first.get(i);
		count++;
	}
	return temp;
}
